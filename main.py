...
dtxt = pandas.read_csv("C:\\Users\\marca\\Documents\\Data-project\\new-data\\my_employees.txt")
 
#Loading the CSV file for writing to a new table in the database
dcsv = pandas.read_csv("C:\\Users\\marca\\Documents\\Data-project\\new-data\my_employees.csv")
 
#Loading the JSON file for writing to a new table in the database
djson = pandas.read_json("C:\\Users\\marca\\Documents\\Data-project\\new-data\\my_employees.json")
 
#Loading the Excel file for writing to a new table in the database
dxlsx = pandas.read_excel("C:\\Users\\marca\\Documents\\Data-project\\new-data\\my_employees.xlsx", sheet_name = 0)
...
engine = create_engine('postgresql+psycopg2://<EnterUsernameHere>:<EnterPasswordHere>@127.0.0.1:5432/staff')
...
dtxt.to_sql('allstaff3', engine, schema = 'mystaff', index = False)
 
dcsv.to_sql('allstaff3', engine, schema = 'mystaff', index = False, if_exists = 'append')
 
djson.to_sql('allstaff3', engine, schema = 'mystaff', index = False, if_exists = 'append')
 
dxlsx.to_sql('allstaff3', engine, schema = 'mystaff', index = False, if_exists = 'append')
 
dsql.to_sql('allstaff3', engine, schema = 'mystaff', index = False, if_exists = 'append')
 
 
#Reading SQL queries to DataFrames and building a HTML summary report
#Printing the entire table of employees
query_all = pandas.read_sql_query('SELECT * FROM mystaff.allstaff3', engine)
 
#The total number of employees
query_count = pandas.read_sql_query('SELECT COUNT (*) FROM mystaff.allstaff3', engine)
total_employees = query_count.iloc[0][0]
#print(total_employees)
 
#The total number of departments
query_dept = pandas.read_sql_query('SELECT COUNT(DISTINCT "Department") FROM mystaff.allstaff3', engine)
total_depts = query_dept.iloc[0][0]
#print(total_depts)
 
#The number of employees per department
query_epd = pandas.read_sql_query('SELECT "Department", COUNT("LastName") FROM mystaff.allstaff3 GROUP BY "Department"', engine)
#print(query_epd)
...
with open("C:\\Users\\marca\\Documents\\Data-project\\summary.html", "w") as f:
